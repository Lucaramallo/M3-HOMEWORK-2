...
-- REPLACE(columna_valores_replace,'sustituir la coma','por un punto')
UPDATE `cliente` SET Latitud = REPLACE(Y,',','.'); 
UPDATE `cliente` SET Longitud = REPLACE(X,',','.');
-- SELECT * FROM `cliente`;
ALTER TABLE `cliente` DROP `Y`; -- elimina columna Y
ALTER TABLE `cliente` DROP `X`; -- elimina columna X
